...
4. Architecture Logicielle (Stack Technique)ComposantTechnologieRôle & Performance MobileFrontendNext.js (Tailwind CSS)PWA : Responsive, fluide, optimisée pour le SEO (SSR).BackendNode.js Traitement des requêtes de filtrage complexe et géolocalisation.BBDDMySQL Indexation spatiale pour les recherches par coordonnées/CP.HébergementPlanetHosterServeur LiteSpeed : Latence minimale pour les recherches locales.DonnéesRefuges PartenairesSource de données interne gérée par les associations.5. Modèle de Données (Schéma Prisma)Extrait de codemodel UserProfile {
